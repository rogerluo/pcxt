...
<%
	...
	If courseid = "" Or Not IsNumeric(courseid) or coursedate = "" Or (target < 0 And target > 1) Then 
		conn.close
		Set conn = Nothing 
...
<%
	If target = 0 Then 
		strsql = "select * from [coursemember_gra] where courseid ='"+courseid+"' and isevaluate=1 and coursedate = '"+coursedate+"'"
	Else 
		strsql = "select * from [coursemember_gra] where courseid ='"+courseid+"' and isevaluate=0 and coursedate = '"+coursedate+"'"
	End If 
