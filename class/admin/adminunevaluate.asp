...
<table width="800" border="1" align="center" cellpadding="5" cellspacing="0" class="tbbg"">
 <tr> 
      <td align="center" class="titlebg" bgcolor='#cccccc'>课程详细信息&nbsp;&nbsp;【<a href="admintotalevaluate.asp?courseid=<%=courseid%>&coursedate=<%=coursedate%>&courseorder=<%=courseorder%>">查看评价统计信息</a>】</td>
 </tr>
 <tr>
	<td>
...
<%
	...
	Do While Not rs.eof
		...
		Else 
			str = "是"
			response.write CStr(i) +"</td><td>"+ rs("stuid") +"</td><td>" + rs("stuname") +"</td><td>" +rs("stuobject")+"</td><td>"+rs("stucollege")+"</td><td>"+str+"</td><td><a href='adminpersonevaluate.asp?courseid="+rs("courseid")+"&stuid="+rs("stuid")+"&coursedate="+cstr(coursedate)+"&courseorder="+cstr(courseorder)+"'>查看</a>"
